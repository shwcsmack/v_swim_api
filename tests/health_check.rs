...
use uuid::Uuid;
use v_swim_api::configuration::{get_configuration, DatabaseSettings};
...
async fn subscribe_returns_a_200_for_valid_data() {
    ...
    let client = reqwest::Client::new();
    let body = "name=le%20quin&email=ursula_le_quin%40gmail.com";
...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    ...
    let connection_pool = PgPool::connect(&config.connection_string())
        .await
        .expect("Failure to connect to postgres");
